# для приготовления напитка обязательно требуется один сорт кофе, обязательно требуется вода, необязательно может быть
# добавлено молоко и необязательно один сироп.
# Если ресурс для приготовления напитка есть в аппарате, но его количества недостаточно чтобы сделать хотя бы одну порцию
# напитка, то напитки где используется этот ресурс не нужно отображать.
# Результатом работы программы будет являться список, в котором перечислены все виды напитков, готовые к приготовлению.
# Элементами списка будут словари с Вот пример такого списка:

# В случае если не удалось найти ни одного напитка который можно было бы приготовить - поднять исключение
# OutOfResourceError. В программе должна быть функция calculate, которая принимает исходный список и возвращает список возможных напитков.
